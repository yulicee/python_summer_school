...
class Spaceship(pygame.sprite.Sprite):
    ...
    def move(self, dx, dy):
        self.rect.x += dx
        self.rect.y += dy
        self.rect.clamp_ip(self.game.screen.get_rect())  # Ensure the spaceship stays within screen bounds
...
class Star(pygame.sprite.Sprite):
    def __init__(self, game):
        super().__init__()
        size = random.randint(5, 15)  # Random size between 5 and 15
        self.image = self.create_star_image(size)
        self.rect = self.image.get_rect()
        self.game = game
        left_bound, right_bound = self.game.get_star_spawn_area()
        self.rect.x = random.randint(left_bound, right_bound - self.rect.width)
        self.rect.y = random.randint(-self.rect.height, HEIGHT // 2)
        self.speed = 2 + (self.game.level - 1) * 0.5  # Increase speed with each level
